...
-- ============================================================
-- ============================================================
-- MÓDULO 6: NOTIFICACIONES Y ALERTAS
-- ============================================================
-- ============================================================

-- ============================================================
-- TABLA: Tipo_Notificacion
-- Catálogo: SESION, TRANSFERENCIA, ALERTA
-- ============================================================
IF OBJECT_ID('dbo.Tipo_Notificacion', 'U') IS NOT NULL
    DROP TABLE dbo.Tipo_Notificacion;
GO

CREATE TABLE dbo.Tipo_Notificacion (
    id_tipo_notificacion INT IDENTITY(1,1) PRIMARY KEY,
    nombre_tipo          VARCHAR(20) NOT NULL CONSTRAINT UQ_TipoNotif UNIQUE
);
GO

INSERT INTO dbo.Tipo_Notificacion (nombre_tipo) VALUES ('SESION');
INSERT INTO dbo.Tipo_Notificacion (nombre_tipo) VALUES ('TRANSFERENCIA');
INSERT INTO dbo.Tipo_Notificacion (nombre_tipo) VALUES ('ALERTA');
GO

-- ============================================================
-- TABLA: Preferencia_Notificacion
-- Guarda el canal preferido del usuario (SMS o CORREO).
-- Una fila por usuario, se actualiza si cambia su preferencia.
-- ============================================================
IF OBJECT_ID('dbo.Preferencia_Notificacion', 'U') IS NOT NULL
    DROP TABLE dbo.Preferencia_Notificacion;
GO

CREATE TABLE dbo.Preferencia_Notificacion (
    id_usuario INT NOT NULL PRIMARY KEY,
    canal      VARCHAR(10) NOT NULL DEFAULT 'CORREO',
    CONSTRAINT FK_PrefNotif_Usuario FOREIGN KEY (id_usuario) REFERENCES dbo.Usuario(id_usuario),
    CONSTRAINT CK_PrefNotif_Canal   CHECK (canal IN ('SMS', 'CORREO', 'PUSH'))
);
GO

-- ============================================================
-- TABLA: Notificacion
-- Registra cada notificación generada para un usuario.
-- id_transferencia: nullable, solo para tipo ALERTA o TRANSFERENCIA
-- ============================================================
...
CREATE TABLE dbo.Notificacion (
    ...
    CONSTRAINT FK_Notif_Usuario     FOREIGN KEY (id_usuario)           REFERENCES dbo.Usuario(id_usuario),
    CONSTRAINT FK_Notif_Tipo        FOREIGN KEY (id_tipo_notificacion)  REFERENCES dbo.Tipo_Notificacion(id_tipo_notificacion),
    CONSTRAINT FK_Notif_Transferencia FOREIGN KEY (id_transferencia)    REFERENCES dbo.Transferencia(id_transferencia),
    CONSTRAINT CK_Notif_Estado      CHECK (estado IN ('PENDIENTE','ENVIADA','FALLIDA')),
    CONSTRAINT CK_Notif_Canal       CHECK (canal IN ('SMS','CORREO','PUSH'))
);
GO

-- ============================================================
-- PROCEDIMIENTO: sp_GuardarPreferenciaNoficacion
-- Inserta o actualiza el canal preferido del usuario.
-- ============================================================
IF OBJECT_ID('dbo.sp_GuardarPreferenciaNotificacion', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_GuardarPreferenciaNotificacion;
GO

CREATE PROCEDURE dbo.sp_GuardarPreferenciaNotificacion
    @id_usuario INT,
    @canal      VARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
    IF EXISTS (SELECT 1 FROM dbo.Preferencia_Notificacion WHERE id_usuario = @id_usuario)
        UPDATE dbo.Preferencia_Notificacion SET canal = @canal WHERE id_usuario = @id_usuario;
    ELSE
        INSERT INTO dbo.Preferencia_Notificacion (id_usuario, canal) VALUES (@id_usuario, @canal);

    SELECT @canal AS canal_guardado;
END;
GO

-- ============================================================
-- PROCEDIMIENTO: sp_ObtenerPreferenciaNotificacion
-- Retorna el canal preferido del usuario (default CORREO).
-- ============================================================
IF OBJECT_ID('dbo.sp_ObtenerPreferenciaNotificacion', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_ObtenerPreferenciaNotificacion;
GO

CREATE PROCEDURE dbo.sp_ObtenerPreferenciaNotificacion
    @id_usuario INT
AS
BEGIN
    SET NOCOUNT ON;
    SELECT ISNULL(
        (SELECT canal FROM dbo.Preferencia_Notificacion WHERE id_usuario = @id_usuario),
        'CORREO'
    ) AS canal;
END;
GO
...
-- ============================================================
-- PROCEDIMIENTO: sp_CrearNotificacion
-- Registra una nueva notificación en la tabla.
-- ============================================================
IF OBJECT_ID('dbo.sp_CrearNotificacion', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_CrearNotificacion;
GO

CREATE PROCEDURE dbo.sp_CrearNotificacion
    @id_usuario           INT,
    @id_tipo_notificacion INT,
    @id_transferencia     INT = NULL,
    @mensaje              VARCHAR(500),
    @canal                VARCHAR(10),
    @estado               VARCHAR(10) = 'ENVIADA'
AS
BEGIN
    SET NOCOUNT ON;
    INSERT INTO dbo.Notificacion
        (id_usuario, id_tipo_notificacion, id_transferencia,
         mensaje, canal, estado, leida, fecha_hora)
    VALUES
        (@id_usuario, @id_tipo_notificacion, @id_transferencia,
         @mensaje, @canal, @estado, 0, GETDATE());
...
-- ============================================================
-- PROCEDIMIENTO: sp_ObtenerNotificacionesPorUsuario
-- Lista todas las notificaciones del usuario, más recientes primero.
-- ============================================================
IF OBJECT_ID('dbo.sp_ObtenerNotificacionesPorUsuario', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_ObtenerNotificacionesPorUsuario;
GO

CREATE PROCEDURE dbo.sp_ObtenerNotificacionesPorUsuario
    @id_usuario INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INNER JOIN dbo.Tipo_Notificacion tn ON n.id_tipo_notificacion = tn.id_tipo_notificacion
...
-- ============================================================
-- PROCEDIMIENTO: sp_MarcarNotificacionLeida
-- Marca una notificación como leída.
-- ============================================================
IF OBJECT_ID('dbo.sp_MarcarNotificacionLeida', 'P') IS NOT NULL
...
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Notificacion SET leida = 1 WHERE id_notificacion = @id_notificacion;
    SELECT @@ROWCOUNT AS actualizados;
END;
GO

-- ============================================================
-- PROCEDIMIENTO: sp_MarcarTodasLeidas
-- Marca todas las notificaciones de un usuario como leídas.
-- ============================================================
IF OBJECT_ID('dbo.sp_MarcarTodasLeidas', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_MarcarTodasLeidas;
GO

CREATE PROCEDURE dbo.sp_MarcarTodasLeidas
    @id_usuario INT
AS
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Notificacion SET leida = 1 WHERE id_usuario = @id_usuario AND leida = 0;
    SELECT @@ROWCOUNT AS actualizados;
END;
GO

-- ============================================================
-- PROCEDIMIENTO: sp_ContarNoLeidas
-- Retorna el número de notificaciones no leídas (para badge).
-- ============================================================
IF OBJECT_ID('dbo.sp_ContarNoLeidas', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_ContarNoLeidas;
GO

CREATE PROCEDURE dbo.sp_ContarNoLeidas
    @id_usuario INT
AS
BEGIN
    SET NOCOUNT ON;
...
-- ============================================================
-- PROCEDIMIENTO: sp_ActualizarEstadoNotificacion
-- Actualiza el estado de una notificación (ENVIADA/FALLIDA).
-- ============================================================
IF OBJECT_ID('dbo.sp_ActualizarEstadoNotificacion', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_ActualizarEstadoNotificacion;
GO

CREATE PROCEDURE dbo.sp_ActualizarEstadoNotificacion
    @id_notificacion INT,
    @nuevo_estado    VARCHAR(10)
AS
BEGIN
    SET NOCOUNT ON;
    UPDATE dbo.Notificacion SET estado = @nuevo_estado WHERE id_notificacion = @id_notificacion;
    SELECT @@ROWCOUNT AS actualizados;
END;
GO

-- ============================================================
-- PROCEDIMIENTO: sp_CancelarTransferenciaPendiente
-- Cancela una transferencia en estado PENDIENTE (alerta no confirmada).
-- ============================================================
IF OBJECT_ID('dbo.sp_CancelarTransferenciaPendiente', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_CancelarTransferenciaPendiente;
GO

CREATE PROCEDURE dbo.sp_CancelarTransferenciaPendiente
    @id_transferencia INT,
    @id_usuario       INT
AS
BEGIN
...
PRINT 'Módulo 6 (Notificaciones y Alertas) creado correctamente.';
GO
